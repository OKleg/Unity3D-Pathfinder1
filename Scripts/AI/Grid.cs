...
public class Grid : MonoBehaviour
{
    ...
    void calculatePath(Vector2Int startNode, Vector2Int finishNode)
    {
        ...
        DijkstraSearch(startNode, finishNode);
        AStarSearch(startNode, finishNode);
        //  Реализуется аналог волнового алгоритма, причём найденный путь не будет являться оптимальным 
        WaveSearch(startNode, finishNode);
    }
    //  BEGIN MY CODE

   
    private void DijkstraSearch(Vector2Int start, Vector2Int goal)
    {
        SearchAlgorithm alg = new SearchAlgorithm(grid);
        alg.Algorithm(start, goal, DijkstraColor, alg.ZeroDistance);
       
    }

    private void AStarSearch(Vector2Int start, Vector2Int goal)
    {
        SearchAlgorithm alg = new SearchAlgorithm(grid);
    ...
    private void WaveSearch(Vector2Int startNode, Vector2Int finishNode)
    {
        //  Реализуется аналог волнового алгоритма, причём найденный путь не будет являться оптимальным 
        ...
        while (nodes.Count > 0)
        {
            ...
            foreach (var node in neighbours)
            {
                ...
                float new_dist = grid[current.x, current.y].Distance + PathNode.Dist(grid[node.x, node.y], grid[current.x, current.y]);
                if (grid[node.x, node.y].Distance > new_dist)
                {
                    dists[node] = new_dist;
